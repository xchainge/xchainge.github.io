...
function getPrice(priceStart, priceConstant, priceExponent, elapsedSeconds) {
  ...
  var price = Math.floor(priceStart * (1 + elapsedSeconds) /
    (1 + elapsedSeconds + decayRate));
  return price;
}
...
function updateCurrentValues(obj) {
  ...
  if (obj['current-price'] !== undefined) {
    val = (obj['current-price']).toLocaleString(
      'en',
  ...
  if (obj['current-target'] !== undefined) {
    var val = obj['current-target'].toLocaleString(
      'en',
  ...
  if (obj['current-total'] !== undefined) {
    var val = obj['current-total'].toLocaleString(
      'en',
      {maximumFractionDigits: 0}
    );
  ...
  if (obj['total-issued'] !== undefined) {
    var val = obj['total-issued'].toLocaleString(
      'en',
      {maximumFractionDigits: 0}
    );
  ...
  if (obj['amount-raised'] !== undefined) {
    var val = obj['amount-raised'].toLocaleString(
      'en',
      {maximumFractionDigits: 0}
    );
  ...
  if (obj['current-target'] !== undefined && obj['amount-raised'] !== undefined) {
    var val = (obj['current-target'] - obj['amount-raised']).toLocaleString(
      'en',
      {maximumFractionDigits: 0}
    );
  ...
  if (obj['current-end'] !== undefined) {
    var val = obj['current-end'].toLocaleString(
      'en-gb', // 24h clock
      {month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit'}
    ),
      tz = obj['current-end'].toLocaleString(
        'en-gb',
        {timeZoneName:'short'}).split(' ').pop();;
...
function getStaticData() {
  $.when($.getJSON('assets/scripts/static.json'), $.getJSON('assets/scripts/status.json'))
...
var countdown = setInterval(function() { ticker(); }, 1000);
...
function checkAlertsForm() {
  ...
  var valid = $('#alerts-form #email').val().match(/@/) &&
    price &&
    grecaptcha.getResponse() &&
    $('#noLiability').is(':checked') &&
